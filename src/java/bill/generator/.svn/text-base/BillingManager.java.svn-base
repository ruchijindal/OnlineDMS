...
public class BillingManager implements Runnable {
    ...
    public double getRate(String connType, String connCategory, String flatType, BigDecimal plotSize, BigDecimal pipeSize, Date rateYear) {
        ...
        try {
            sql = "select *  from Rates where (pipe_size =?) and (conn_type =?) and (date_from < ? and date_to > ?) and (conn_category=?) and  (min_plot_size < ? and max_plot_size > ?)";
            pst = con.prepareStatement(sql);
            pst.setBigDecimal(1, pipeSize);
            pst.setString(2, connType);
            pst.setDate(3, new java.sql.Date(rateYear.getTime()));
            pst.setDate(4, new java.sql.Date(rateYear.getTime()));
            pst.setString(5, connCategory);
            pst.setBigDecimal(6, plotSize);
            pst.setBigDecimal(7, plotSize);
    ...
    public double getRateForBulk(String connType, String connCategory, String flatType, BigDecimal pipeSize, Date rateYear) {
        ...
        double rate = 0.0;
        try {
            sql = "select *  from Rates where (pipe_size =?) and (conn_type =?) and (date_from < ? and date_to > ?) and (conn_category=?) ";
            pst = con.prepareStatement(sql);
            pst.setBigDecimal(1, pipeSize);
            pst.setString(2, connType);
            pst.setDate(3, new java.sql.Date(rateYear.getTime()));
    ...
    public double getRateForFlat(String connType, String connCategory, String flatType, int pipeSize, String sector, Date rateYear) {
        ...
        double rate = 0.0;
        try {
            sql = "select * from Rates where (conn_type =?) and (date_from < ?  and date_to > ?) and (conn_category=?) and (trim(sector) = ?) and (flat_type =  ?) ";
            pst = con.prepareStatement(sql);

            pst.setString(1, connType);
            pst.setDate(2, new java.sql.Date(rateYear.getTime()));
            pst.setDate(3, new java.sql.Date(rateYear.getTime()));
            pst.setString(4, connCategory);
            pst.setString(5, sector.trim());
            pst.setString(6, flatType);
    ...
    public double getInterestRate(Date rateDate) {
        ...
        double rate = 0.0;
        try {
            sql = "select * from Interest_Rates where date_from <=? and date_to >= ?";
            pst = con.prepareStatement(sql);


            pst.setDate(1, new java.sql.Date(rateDate.getTime()));
            pst.setDate(2, new java.sql.Date(rateDate.getTime()));
   ...
   public Double getCessRate(Date cessDate,double pipeSize, String conType){
        double rate = 0.0;
        try {
            sql = "select * from Cess_Rate where date_from <=? and date_to >= ? and pipe_size = ? and con_type like ?" ;
            pst = con.prepareStatement(sql);


            pst.setDate(1, new java.sql.Date(cessDate.getTime()));
            pst.setDate(2, new java.sql.Date(cessDate.getTime()));
            pst.setDouble(3, pipeSize);
            pst.setString(4, "%"+conType+"%");
